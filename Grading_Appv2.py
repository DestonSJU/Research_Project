...
tools = [java_compiler]
llm = ChatOllama(model="llama3.1:8b", temperature=0.1, top_p=0.1).bind_tools(tools)
structure_llm = ChatOllama(model="llama3.1:8b", temperature=0.1, top_p=0.1).with_structured_output(ToolCall)
...
def code_node(state: AgentState):
    ...
    for m in messages:
        if isinstance(m, ToolMessage):
            ...
            return {"messages": [response], "output": AIMessage(content=m.content), "called": True}
...
def code_edge(state: AgentState):
...
def syntax_node(state: AgentState):
    ...
    system_prompt = SystemMessage(
        content="Fix the syntax errors in the code. Your response should be in this format: \n"
...
def syntax_edge(state: AgentState):
    last_message = state['messages'][-1]
    if "0" or "no" or "none" in last_message.content:
        return END
    else:
        return "code"
...
workflow.add_conditional_edges("code", code_edge)
#workflow.add_conditional_edges("agent", should_continue)
workflow.add_edge("tools", "syntax")# Loops back after tool execution
workflow.add_conditional_edges("syntax", syntax_edge)
...
def run_model_button():
    with st.spinner("Executing Local Workflow..."):
        result = app.invoke({"messages": [], "rubric": rubric_text, "code": file_input.read().decode("utf-8"), "syntax_errors": False, "called": False, "output":AIMessage(content="")})
        # Displays the output from the code and the LLM's response to it
        for m in result["messages"][:-1]:
            if isinstance(m, ToolMessage):
                st.session_state.chat_history.append(m.content)
            else:
                st.session_state.chat_history.append(m)
        final_answer = result["messages"][-1].content
        st.session_state.chat_history.append(AIMessage("Final Output: \n" + final_answer + "\n Tool Called: " + str(result["called"]) + "Output: " + result["output"].content))
